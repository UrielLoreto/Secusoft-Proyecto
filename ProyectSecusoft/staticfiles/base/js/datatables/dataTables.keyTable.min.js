...
(function(f) {
    ...
    f.extend(k.prototype, {
        ...
        _constructor: function() {
            ...
            f(b.table().body()).on("click.keyTable", "th, td", function() {
                if (!1 !== a.s.enable) {
                    var c = b.cell(this);
                    c.any() && a._focus(c)
                }
            });
            f(i).on("keydown.keyTable", function(b) {
                a._key(b)
            });
            if (this.c.blurable) f(i).on("click.keyTable", function(c) {
                f(c.target).parents(".dataTables_filter").length && a._blur();
                f(c.target).parents().filter(b.table().container()).length || f(c.target).parents("div.DTE").length || a._blur()
            });
            if (this.c.editor) b.on("key.keyTable", function(b,
                c, d, f, i) {
                a._editor(d, i)
            });
            ...
            b.on("destroy.keyTable", function() {
                ...
                f(i.body).off("keydown.keyTable").off("click.keyTable")
            });
            var c = b.state.loaded();
            if (c && c.keyTable) b.one("init",
                function() {
        ...
        _editor: function(a, b) {
            ...
            var e = f("div.DTE input, div.DTE textarea");
            e.length && e[0].select();
        ...
        _focus: function(a, b) {
            ...
            if (this.s.enable) {
                if ("number" !== typeof a) {
                    ...
                    e.serverSide && (a += e.start)
                }
                if (-1 !== e.length && (a < e.start || a >= e.start + e.length)) this.s.focusDraw = !0, c.one("draw", function() {
                    d.s.focusDraw = !1;
                    d._focus(a, b)
                }).page(Math.floor(a / e.length)).draw(!1);
                    ...
                    if (l) {
                        if (l.node() === e.node()) return;
                        this._blur()
                    }
                    l = f(e.node());
                    l.addClass(this.c.className);
                    this._scroll(f(h),
                        f(i.body), l, "offset");
                    j = c.table().body().parentNode;
                    j !== c.table().header().parentNode && (j = f(j.parentNode), this._scroll(j, j, l, "position"));
                    this.s.lastFocus = e;
                    this._emitEvent("key-focus", [this.s.dt, e]);
                    c.state.save()
                }
            }
        },
                ...
                if (b) {
                    ...
                    if (!(this.c.keys && -1 === f.inArray(a.keyCode, this.c.keys))) switch (a.keyCode) {
                        ...
                        case 34:
                            ...
                            var e = c.cells({
                                page: "current"
                            }).nodes().indexOf(b.node());
                            c.one("draw", function() {
                                var a = c.cells({
                                    page: "current"
                                }).nodes();
                                d._focus(c.cell(e < a.length ? a[e] : a[a.length - 1]))
        ...
        _scroll: function(a, b, d, c) {
            var c = d[c](),
                ...
                i = a.height(),
                a = a.width();
            c.top < f && b.scrollTop(c.top);
            c.left < j && b.scrollLeft(c.left);
            c.top + e > f + i && b.scrollTop(c.top + e - i);
            c.left + d > j + a && b.scrollLeft(c.left + d - a)
        },
        _shift: function(a, b, d) {
            ...
            if (j) {
                var h = c.rows({
                    filter: "applied",
                    order: "applied"
                }).indexes().indexOf(j.index().row);
                e.serverSide && (h += e.start);
                c = c.columns(g).indexes().indexOf(j.index().column);
                e = g[c];
                "right" === b ? c >= g.length - 1 ? (h++, e = g[0]) : e = g[c + 1] : "left" === b ? 0 === c ? (h--, e = g[g.length - 1]) : e = g[c - 1] : "up" === b ? h-- : "down" === b && h++;
                0 <= h && h < i && -1 !== f.inArray(e, g) ? (a.preventDefault(), this._focus(h, e)) : !d || !this.c.blurable ? a.preventDefault() : this._blur()
            }
        },
        _tabInput: function() {
            var a = this,
            ...
            if (-1 != d) f('<div><input type="text" tabindex="' + d + '"/></div>').css({
                position: "absolute",
                height: 1,
                width: 0,
                overflow: "hidden"
    ...
    k.version = "2.1.1";
    f.fn.dataTable.KeyTable = k;
    f.fn.DataTable.KeyTable = k;
    g.Api.register("cell.blur()",
        function() {
    ...
    g.ext.selector.cell.push(function(a, b,
        d) {
        var b = b.focused,
            a = a.keytable,
            c = [];
        if (!a || b === m) return d;
        for (var e = 0, f = d.length; e < f; e++)(!0 === b && a.focused(d[e]) || !1 === b && !a.focused(d[e])) && c.push(d[e]);
        return c
    });
    f(i).on("preInit.dt.dtk", function(a, b) {
        if ("dt" === a.namespace) {
            var d = b.oInit.keys,
                c = g.defaults.keys;
            if (d || c) c = f.extend({}, d, c), !1 !== d && new k(b, c)
        }
    });
    return k
});